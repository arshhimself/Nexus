...
export default function ProctoredTestPage() {
  ...
  const handleSubmitQuestion = async () => {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        const analyzeRes = await fetch(`http://${process.env.NEXT_PUBLIC_FASTAPI_URL}/analyze`, {
          method: "POST",
        ...
        const submitRes = await fetch(`http://${process.env.NEXT_PUBLIC_DJANGO_URL}/api/quiz/submit/`, {
          method: "POST",
          headers: {
  ...
  const uploadToServer = async (file) => {
    ...
    try {
      const res = await fetch(`http://${process.env.NEXT_PUBLIC_FASTAPI_URL}/upload/s3/`, {
        method: "POST",
        body: formData,
      });
